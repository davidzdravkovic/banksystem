...
#include <iostream>
  ...
  class User;
   double CreditAccount::deposit(pqxx::connection &conn,double amount,int accountNumber) {
    if(amount<0) {
      return -1;
    }
    try{
      ...
      pqxx::result r = txn.exec_params(
            "SELECT balance, total_deposit FROM credit_accounts WHERE account_number = $1",
           accountNumber
        );
...
double CreditAccount :: withdraw(pqxx::connection &conn,double amount,int accountNumber) {
    ...
    try{
      pqxx::work txn(conn);
         ...
         pqxx::result r1 = txn.exec_params(
            "SELECT  balance,total_withdraw FROM credit_accounts WHERE account_number = $1",
            accountNumber);
...
return -1;
   ...
   std::shared_ptr<Account> CreditAccount:: createAccount (pqxx::connection &conn,std::shared_ptr<Account> account,int number) {
   auto cAccount=std::dynamic_pointer_cast<CreditAccount>(account);
   if(!cAccount) {
...
if (hasThisType) {
 ...
 std::string query = "INSERT INTO accounts (personal_id,account_number,account_type,full_name) VALUES "
  ...
  txn.exec_params(query,number,cAccount->getAccountNumber(),cAccount->getAccountType(),cAccount->getOwnerName());
   std::string query1 = "INSERT INTO credit_accounts (personal_id,account_number,account_type,full_name) VALUES "
    "($1,$2,$3,$4)";
     txn.exec_params(query1,number,cAccount->getAccountNumber(),cAccount->getAccountType(),cAccount->getOwnerName());
     pqxx::result r2=txn.exec_params("Select * FROM ONLY credit_accounts WHERE account_number=$1",cAccount->getAccountNumber());
     txn.commit();
